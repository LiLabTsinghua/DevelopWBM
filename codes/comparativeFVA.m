...
[~,ecFluxDist,~]   = fixObjective1(ecModel,false);
...
model.ub(objIndx) = 1.02*priorValue;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [OptimalValue, optFluxDist, model] = fixObjective1(model,fixed,priorValue)
% Optimize and fixes objective value for GEM
objIndx  = find(model.c~=0);
if nargin < 3
    sol = solveLP(model);
...
model.ub(objIndx) = factor*priorValue;
model.lb(objIndx) = 1.0001*priorValue;
