function [model, hasEffect, constrRxnNames, deletedGenes] = deleteHumanGenes(model, geneList, downRegFraction)
  ...
  model.genes(geneInd) = strcat(model.genes(geneInd),'_deleted');
    ...
    if (~isempty(rxnInd))
        ...
        for j = 1:length(rxnInd)
            if (isfield(model, 'grRules') && ~isempty(model.grRules{rxnInd(j)})) %To avoid errors if the rule is empty
                if (~isempty(model.grRules{rxnInd(j)}))
                    constrainRxn(j) = true;
                end
            end
        end
