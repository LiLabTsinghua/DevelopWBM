...
def Data_plot(x,y):
    ...
    ax.plot(True_energy, predicted_energy, 'o', color = '#5ab4ac', markersize = 5, alpha=1)
    ...
    plt.ylabel('Predicted BMR by equation (kcal)', fontsize=8)

    minorticks_off()

    plt.savefig("../../results/Figures/Support_Fig_7d_female_BMR_Function.pdf", dpi=400, bbox_inches = 'tight')
    plt.show()

def main():
    df_corr = pd.read_csv('../../data/AdultFemale_BMR.tsv', sep = '\t')
    
    x = df_corr['BMR (kcal)'].values.tolist()
    y = df_corr['Prediction of Mifflin and St Jeor equations'].values.tolist()
