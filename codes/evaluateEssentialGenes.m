function results = evaluateEssentialGenes(eGenes, datasetName)

% Evaluate and compare experimental fitness genes with essentiality results
% predicted from 5 cell-line specific GEMs from Hart 2015 datasets or 625
% cell-line specific GEMs from  DepMap.
%
% Input:
%
...
if strcmp(datasetName,'Hart2015')
    ...
    x = readtable('../data/eGenesData/Hart2015_TableS2_add_ID.xlsx');
    ...
    genes = x.Gene_id;
    celltypes = regexprep(upper(x.Properties.VariableNames(4:8)'),'BF_','');
...
elseif strcmp(datasetName,'DepMap')
    ...
    cell_col = setdiff(cell_col,'Var1');  % skip the gene column
    ...
    clear x genes celltypes col_types cell_col
end
...
if strcmpi(datasetName,'Hart2015')
    tissues(end+1) = {'all'};  % also include "all" category for Hart2015 dataset
else
    expdata.essential = double(expdata.gene_effect < DepMapThresh);
end
