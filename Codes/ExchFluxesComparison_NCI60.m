...
% Jiahao Luo.      Last edited: 2024-04-07
...
for i=1:length(cellLines)
    ...
    mkdir (['../models/NCI60/' cellLines{i} '/Results'])
    disp(cellLines{i})
    %Load models
    load(['../models/NCI60/' cellLines{i} '/ecModel_' cellLines{i} '.mat'])
    load(['../models/NCI60/' cellLines{i} '/' cellLines{i} '.mat'])
    eval(['model =' cellLines{i} ';'])
    model.b = zeros(length(model.mets),1);
    
    ecModel_batch = ChangeAbnormalProMass(ecModel_batch);
    
    %Allow any level of average enzyme saturaion (0 - 100%)
    P_index = strcmpi(ecModel_batch.rxnNames,'prot_pool_exchange');   
    ecModel_batch.lb(P_index) = -0.593;
    
    % Total_mass = ecModel_batch.lb(P_index);
    %Save models
    ...
    if ~isempty(ecMsol.x) & ~isempty(GEMsol.x)
        ...
        ecModel_batch  = setHamsMedium(ecModel_batch,false);
        ecModel_batch.lb(P_index) = -0.593;
        % ecModel_batch.lb(P_index) = Total_mass;
...
%Fix objective value
model.ub(objIndex) = 1.01*objValue;
model.lb(objIndex) = 0.99*objValue;
...
% protIndx          = find(strcmpi(model.rxns,'MAR10062'));
model.c(protIndx) = -1;
% model.c(protIndx) = 1;
...
if direction <0
    if ecFlag
        ...
        model.ub(index) = lowValue*abs(value);
    else
        model.lb(index) = value;
...
if nargin <3 
    ...
    gIndex    = find(strcmpi(model.rxns,'MAR13082'));
    protIndex = find(strcmpi(model.metNames,'protein_pool_biomass'));
    % protIndex = find(strcmpi(model.metNames,'prot_pool'));
    ...
    if constrainPool
        ...
        model.lb(protPool) = -Ptot;
    end
end
end
%--------------------------------------------------------------------------
function temp_model = ChangeAbnormalProMass(ecmodel)

temp_model = ecmodel;
Protein_mass = [];
for i = 1: length(temp_model.rxns)
    p = findIndex(temp_model.mets,'prot_pool');
    Protein_mass(i,1) = temp_model.S(p,i);
end

wrong_rxn_index = find(Protein_mass<-100);
wrong_rxn_mass = Protein_mass(wrong_rxn_index);
wrong_rxn_mass1 = wrong_rxn_mass*0.001;

temp_model.S(p,wrong_rxn_index) = wrong_rxn_mass1;
