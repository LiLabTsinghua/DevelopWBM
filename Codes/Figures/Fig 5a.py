...
# filepath = 'D:\\All_Human_GTEx\\Food\\food_data\\'
# df_food = pd.read_excel(filepath+'food_sorted_data.xlsx')
...
def Food_energy(Food_id, True_energy, predicted_energy):
    ...
    correlation = np.corrcoef(predicted_energy_, r_line)[0,1]  #相关系数
    R2 = correlation**2
    ...
    plt.scatter(Cornstarch_energy_x, Cornstarch_predicted_energy, s = 10, alpha = 0.8, color = '#fdae61')
    plt.scatter(Fish_energy_x, Fish_predicted_energy, s = 10, alpha = 0.8, color = '#b2182b')
    plt.scatter(Butter_energy_x, Butter_predicted_energy, s = 10, alpha = 0.8, color = '#762a83')
    ...
    plt.savefig("D:/All_Human_GTEx/Food/food_data/Figure(EC_model_ATPM3_3).pdf", dpi=400, bbox_inches = 'tight')
    plt.show()


filepath = 'D:\\All_Human_GTEx\\Food\\food_data\\'
df_food = pd.read_excel(filepath+'food_sorted_data(WBM_ATPM3_4).xlsx')
temp_Food_id = df_food['food_id'].values.tolist()
Food_id = [str(i) for i in temp_Food_id]
True_energy = df_food['Energy'].values.tolist()
predicted_energy = df_food['Energy_new(WBM_ATPM3_4)'].values.tolist()
